...
**Answer: a lot.** Raising `imgsz` from 640 to 1024 on identical data and training budget lifted
mAP50 by +43% (0.258 to 0.370), with the gains ranked exactly where the hypothesis
...
from altitude is 7-15 pixels, and after resizing to 640 it is often below the stride.
...
Two YOLO11n models, identical in every way except `imgsz` (640 vs 1024): same 2,000-image
train subset (seed=42), full val set, SGD lr0=0.01 pinned, same augmentation, same budget.
...
*Left to right: ground truth · 640 model · 1024 model. On the distant path the 640 model
...
2. **Lookalike twins:** with enough pixels to see, the model now *hesitates*: motor/bicycle
   confusion is perfectly symmetric (134:134), and pedestrian-to-people confusions jumped
   from 30 to 263. False positives rose 14% vs the baseline. The bottleneck moved from
   "can't see" to "can't tell apart", a problem more resolution cannot fix.
...
- **Gentler scale augmentation (0.5 to 0.2 at 1024), which ran and came back
