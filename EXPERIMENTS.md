...
| highres_1024_2k | identical to baseline except imgsz=1024, batch=6, workers=2 | 0.369 | 0.216 | ~3:20 | **Hypothesis confirmed decisively: mAP50 +43% (0.258→0.369) on identical data and budget.** Early-stopped at ep72 (best ep47) — converged; longer same-config training would add nothing. Every class improved, gains ranked inversely by object size: motor +0.165, pedestrian +0.161 (+58%), tricycle +0.142, people +0.113, bicycle 0.046→0.125 (×2.7); >64px size bucket unchanged at 92.7% — a perfect internal control. Detection rate <8px: 13.6%→28.4% (×2.1); overall class-agnostic detection 46.4%→58.9%. mAP50-95 rose relatively more (+52%) — boxes are both found more and localized better. Cost: inference 1.6→6.9ms (×4.3). New bottleneck exposed: FP 5.7k→8.8k and symmetric twin-class confusion (motor↔bicycle 134:134, pedestrian→people 30→263) — the model now sees enough pixels to hesitate between lookalikes. Official numbers from scripts/compare_runs.py (unified val protocol on best.pt); note the epoch-table numbers differ (0.322) — validation protocol matters. |
...
- **Official metrics come from scripts/compare_runs.py** — a unified val protocol on best.pt with
  explicit data/imgsz. End-of-training console tables use training-time settings and read lower
  (0.322 vs 0.369 for the 1024 run); never mix the two sources.
...
- bicycle: improved ×2.7 (AP50 0.125) but still worst; F1 peaks at only ~0.21 at low confidence.
